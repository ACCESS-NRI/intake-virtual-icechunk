...
def pl_search(
    *,
    ...
    iterable_dtypes = iterable_dtypes or {
        colname: tuple for colname in columns_with_iterables
    }
    ...
    for colname, subquery in query_non_iterable.items():
        ...
        if is_pattern(subquery):
            ...
            subquery = [
                f"(?i){q}" if ci else q for q, ci in zip(subquery, case_insensitive)
            ]

            lf = lf.filter(
                pl.col(colname).str.contains("|".join(subquery), literal=False)
            )
        else:
            lf = lf.filter(pl.col(colname).is_in(subquery, nulls_equal=True))

    query_iterable = {
        key: val for key, val in query.items() if key in columns_with_iterables
    }
    for colname, subquery in query_iterable.items():
        if is_pattern(subquery):
            ...
            lf = lf.filter(
                pl.col(colname)
                .list.eval(pl.element().is_in(subquery, nulls_equal=True))
                .list.any()
            )
